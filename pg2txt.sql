...
select 'Database :' as item, current_database() as value
union all
...
select datname, oid::text, datdba::regrole::text,
...
(select nspname as schema, rolname as owner,
 ...
 sum(case when relkind='p' THEN 1 ELSE 0 end) as part_table,
 sum(case when relkind='I' THEN 1 ELSE 0 end) as part_idx,
 sum(case when relkind='v' THEN 1 ELSE 0 end) as view,
 sum(case when relkind='S' THEN 1 ELSE 0 end) as sequence,
 sum(case when relkind='c' THEN 1 ELSE 0 end) as comp_type,
 sum(case when relkind='f' THEN 1 ELSE 0 end) as foreing_tab,
 ...
 sum(case when relpersistence='u' THEN 1 ELSE 0 end) as unlogged,
 sum(case when relpersistence='t' THEN 1 ELSE 0 end) as temporary
...
select spcname as tablespace,
 to_char(sum(case when relkind='r' THEN 1 ELSE 0 end),'999G999G999G999G999G999') as tables,
 to_char(sum(case when relkind='r' THEN greatest(reltuples,0) ELSE 0 end),'999G999G999G999G999G999') as rows,
 to_char(trunc(sum(case when relkind='r' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999') as tab_KB,
 to_char(trunc(sum(case when relkind='i' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999') as idx_KB,
 to_char(trunc(sum(case when relkind='t' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999') as toast_KB,
 to_char(trunc(sum(cast(1 as bigint)* relpages *8)),'999G999G999G999G999G999') as size_KB
from pg_class
...
(select nspname as schema, rolname as owner,
 to_char(sum(case when relkind='r' THEN 1 ELSE 0 end),'999G999G999G999G999') as tables,
 to_char(sum(case when relkind='r' THEN greatest(reltuples,0) ELSE 0 end),'999G999G999G999G999G999') as rows,
 to_char(trunc(sum(case when relkind='r' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999') as tab_KB,
 to_char(trunc(sum(case when relkind='i' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999') as idx_KB,
 to_char(trunc(sum(case when relkind='t' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999') as toast_KB,
...
select 'TOTAL', '',
 ...
 to_char(sum(case when relkind='r' THEN reltuples ELSE 0 end),'999G999G999G999G999G999'),
 to_char(trunc(sum(case when relkind='r' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999'),
 to_char(trunc(sum(case when relkind='i' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999'),
 to_char(trunc(sum(case when relkind='t' THEN cast(1 as bigint)* relpages *8 ELSE 0 end)),'999G999G999G999G999G999'),
 to_char(trunc(sum(cast(1 as bigint)* relpages *8)),'999G999G999G999G999G999')
from pg_class;

select count(*) as tables, max(last_autovacuum)::TEXT as last_autovacuum, max(last_vacuum)::TEXT as last_vacuum,
 max(last_autoanalyze)::TEXT as last_autoanalyze, max(last_analyze)::TEXT as last_analyze
...
select schemaname||'.'||relname as table,
 ...
 last_autoanalyze::TEXT as last_autoanalyze, last_analyze::TEXT as last_analyze
...
SELECT schemaname||'.'||tblname as table,
    FILLFACTOR,
...
select schemaname  as schema,
  ...
  n_tup_hot_upd,
  n_tup_del,
  idx_scan*100/nullif(idx_scan+seq_scan,0) as idx_hit_ratio,
  n_tup_hot_upd*100/nullif(n_tup_upd,0) as hot_hit_ratio
 from pg_stat_user_tables
...
select schemaname as schema,
  ...
  toast_blks_hit*100/nullif(toast_blks_read+toast_blks_hit,0) as toast_hit_ratio
...
select o.rolname as owner, n.nspname as schema, t.typname, count(*)
...
select archived_count, last_archived_wal, last_archived_time, failed_count,
       last_failed_wal, last_failed_time, stats_reset,
        ...
        CAST (archived_count AS NUMERIC) / EXTRACT (EPOCH FROM age(now(), stats_reset)) as wals_ps
  from pg_stat_archiver;
select name as replication_parameter, setting
 from pg_settings
...
select name as extension_name, default_version, installed_version,comment
